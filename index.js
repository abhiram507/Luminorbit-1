...
const ORCH_CONFIG = {
  ...
  get apiUrl() {
    ...
    const u = (window.LUMINORBIT_CONFIG && window.LUMINORBIT_CONFIG.API_BASE_URL)
      ...
      || 'https://luminorbitbackend-production-117f.up.railway.app';
    ...
    if (u && u.startsWith('http://') && !u.includes('localhost') && !u.includes('127.0.0.1')) {
      _orchWarn('[Config] Forcing HTTPS for non-localhost URL:', u);
...
const TOOL_REGISTRY = {
  ...
  'Aspect Ratio Converter':            { pipeline: 'enhancement',           preset: 'aspect_ratio' },
  'PNG Converter':                     { pipeline: 'basic',           preset: 'convert_png' },
  'Image Converter':                   { pipeline: 'enhancement',           preset: 'convert' },
  'Watermark Maker':                   { pipeline: 'enhancement',           preset: 'watermark' },
  'Text Adder':                        { pipeline: 'enhancement',           preset: 'text_overlay' },
  'Meme Maker':                        { pipeline: 'enhancement',           preset: 'meme' },
  'Collage Maker':                     { pipeline: 'basic',           preset: 'collage' },
  'Photo Stitcher':                    { pipeline: 'basic',           preset: 'stitch' },
  'Frame Generator':                   { pipeline: 'enhancement',           preset: 'frame' },
  'Passport Image Pro':                { pipeline: 'enhancement',           preset: 'passport' },
  'Threshold':                         { pipeline: 'enhancement',           preset: 'threshold' },
  'Binarize':                          { pipeline: 'enhancement',           preset: 'binarize' },
  'Image Splitter':                    { pipeline: 'basic',           preset: 'split' },
  'Photo Splitter':                    { pipeline: 'enhancement',           preset: 'split' },

  // ── Tools in HTML TOOL_CAPABILITY_MAP missing from TOOL_REGISTRY ──────────
  'Denoiser':                         { pipeline: 'denoising',     preset: 'denoise' },
  ...
  'Texturizer':                       { pipeline: 'enhancement',  preset: 'style_default' },
  'Film Grain':                       { pipeline: 'style-transfer',  preset: 'vintage' },
  'Faded Film':                       { pipeline: 'style-transfer',  preset: 'vintage' },
  'Film Burn Effect':                 { pipeline: 'style-transfer',  preset: 'vintage' },
  'Duotone':                          { pipeline: 'style-transfer',  preset: 'style_default' },
  'Dual Tone Grade':                  { pipeline: 'style-transfer',  preset: 'style_default' },
  'Gradient Map Tool':                { pipeline: 'style-transfer',  preset: 'style_default' },
  'Glitch Generator':                 { pipeline: 'style-transfer',  preset: 'glitch' },
  'Emboss':                           { pipeline: 'style-transfer',  preset: 'sketch' },
  'Resize Pro':                       { pipeline: 'enhancement',           preset: 'resize' },
  'Resize Guru':                      { pipeline: 'enhancement',           preset: 'resize' },
  'Image Resizer Pro':                { pipeline: 'enhancement',           preset: 'resize' },
  'Rotate 90':                        { pipeline: 'enhancement',           preset: 'rotate' },
  'Flip & Rotate':                    { pipeline: 'enhancement',           preset: 'flip' },
  'Focus Tool':                       { pipeline: 'enhancement',           preset: 'blur' },
  'Lens Blur':                        { pipeline: 'enhancement',           preset: 'blur' },
  'Image Stitcher':                   { pipeline: 'enhancement',           preset: 'stitch' },
  'Image Overlay':                    { pipeline: 'enhancement',           preset: 'collage' },
  'Layer Editor':                     { pipeline: 'enhancement',           preset: 'collage' },
  'FrameIt':                          { pipeline: 'enhancement',           preset: 'frame' },
  'Clip Splitter Tool':               { pipeline: 'temporal',      preset: 'trim' },
  'J-Cut Master':                     { pipeline: 'temporal',      preset: 'trim' },
  'L-Cut Pro':                        { pipeline: 'temporal',      preset: 'trim' },
  'Multi Clip Timeline Tool':         { pipeline: 'temporal',      preset: 'merge' },
  'Split Screen Sync':                { pipeline: 'color-matching',      preset: 'merge' },
  'Storyboard Sync':                  { pipeline: 'color-matching',      preset: 'merge' },
};
...
const FetchEngine = (() => {
    ...
    for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
      try {
        ...
        if (attempt <= maxRetries) {
          ...
          _orchWarn(`[FetchEngine] Attempt ${attempt} failed — retrying in ${Math.round(delay)}ms:`, e.message || e);
          await _sleep(delay);
        }
      }
    }
  ...
  async function uploadSafeFetch(url, payload, fileObj, timeoutMs = ORCH_CONFIG.TIMEOUT_UPLOAD) {
    ...
    return retryFetch(cleanUrl, {
      method:  'POST',
      // CORS FIX: stripped Authorization header — causes preflight on Railway
  ...
  async function asyncPollingFetch(jobId, onProgress, abortSignal) {
    ...
    const headers  = {}; // CORS FIX: no Authorization header on poll requests
    ...
    for (let i = 0; i < ORCH_CONFIG.POLL_MAX_ATTEMPTS; i++) {
      ...
      try {
        ...
        if (!resp.ok) {
          _orchWarn(`[Polling] HTTP ${resp.status} on job ${jobId} — continuing`);
          continue;
        }
        data = await resp.json();
      } catch (e) {
        _orchWarn(`[Polling] Network error on attempt ${i + 1}:`, e.message);
        continue;
      }
      ...
      if (data.status === 'completed') {
        ...
        _orchLog(`[Polling] ✓ Job ${jobId} completed after ${i + 1} polls`);
        return out;
      }
...
const ResponseValidator = {
  ...
  _validateOutputValue(output, outputType, toolName) {
    ...
    if (output.startsWith('http') && output.includes('error') && output.length < 60) {
      _orchWarn('[ResponseValidator] Output URL looks like an error path:', output);
    }
  ...
  async build(toolName, userParams, inputFile) {
    ...
    if (inputFile) {
      ...
      if (!mimeCheck.valid) {
        ...
        _orchWarn('[PayloadNormalizer] MIME warning (non-blocking):', mimeCheck.error);
      }
    ...
    if (pipeline && pipeline.needsFile && !inputFile) {
      ...
      _orchWarn(`[PayloadNormalizer] "${toolName}" needs a file input but none provided`);
    }
...
const PipelineRouter = {
  ...
  async execute(payload) {
    ...
    for (const backendBase of backends) {
      ...
      _orchLog(`[Router] → ${payload.tool} | cap=${payload.capability} | backend=${backendBase.replace(/https?:\/\//, '')}`);
      ...
      try {
        const resp = await FetchEngine.retryFetch(
          ...
          {
            method:  'POST',
            // CORS FIX: Only Content-Type header — avoids preflight OPTIONS request.
            // Authorization / X-Pipeline / X-Request-Id were triggering Railway CORS
            // preflight failures on every single tool call. Stripped permanently.
            headers: { 'Content-Type': 'application/json' },
        ...
        _orchLog(`[Router] ✓ ${payload.tool} → provider=${data.provider || 'unknown'} ms=${data.execution_ms || '?'} fallback=${data.fallback_used}`);
        ...
        _orchWarn(`[Router] ✗ ${payload.tool} on ${backendBase.replace(/https?:\/\//, '')}: ${e.message}`);
...
const AsyncJobManager = {
  ...
  async poll(jobId, toolName, onProgress) {
    ...
    try {
      ...
      if (e.code === 'POLLING_ABORTED') {
        _orchLog(`[AsyncJobManager] Job ${jobId} polling was cancelled`);
        return null;
      }
      _orchWarn(`[AsyncJobManager] Job ${jobId} failed:`, e.message);
      return null;
    } finally {
  ...
  cancel(jobId) {
    ...
    if (entry && entry.ctrl) {
      entry.ctrl.abort();
      this._active.delete(jobId);
      _orchLog(`[AsyncJobManager] Cancelled job ${jobId}`);
    }
  },
...
const ResultDispatcher = {
  ...
  dispatch(output, toolName, responseObj, outputType) {
    if (!output) {
      _orchWarn('[ResultDispatcher] No output to render for:', toolName);
      return false;
    }
    ...
    _orchLog(`[ResultDispatcher] Rendering ${resolvedType} for "${toolName}"`);
    ...
    try {
      switch (resolvedType) {
        ...
        default:
          ...
          _orchWarn('[ResultDispatcher] Unknown output type, attempting image render:', resolvedType);
  ...
  _renderImage(src, toolName) {
    ...
    if (img) {
      ...
      img.onerror = () => {
        _orchWarn('[ResultDispatcher] Image failed to load — URL may be expired:', src.substring(0, 80));
      ...
      img.onload = () => {
        img.style.opacity = '1';
        _orchLog(`[ResultDispatcher] Image loaded OK — ${img.naturalWidth}×${img.naturalHeight}px for "${toolName}"`);
      };
    }
  ...
  _renderAudio(src, toolName) {
    ...
    if (aud && aud.tagName === 'AUDIO') {
      ...
      _orchWarn('[ResultDispatcher] No audio element found — storing for export');
    }
  ...
  _renderText(raw, toolName) {
    ...
    _orchLog('[ResultDispatcher] Text rendered:', text.length, 'chars');
  },
...
window.executeOrchestrated = async function executeOrchestrated(toolName, userParams, inputFile) {
  if (!toolName) {
    _orchWarn('[Orchestration] executeOrchestrated called without toolName');
    return false;
  }
  ...
  _orchLog(`[Orchestration] START tool="${toolName}" execId=${execId}`);
  ...
  try {
    ...
    const reg = TOOL_REGISTRY[toolName];
    if (!reg) {
      _orchWarn(`[Orchestration] "${toolName}" not in TOOL_REGISTRY — using basic pipeline`);
    }
    ...
    _orchLog(`[Orchestration] pipeline=${pipelineName} cap=${capability}`);
    ...
    try {
      ...
      if (normErr instanceof OrchError) {
        _orchWarn('[Orchestration] Payload normalization failed:', normErr.message);
    ...
    _orchLog(`[Orchestration] Payload built — file=${payload.file_data ? 'YES' : 'NO'} mime=${payload.file_mime}`);
    ...
    try {
      ...
      _orchWarn('[Orchestration] Backend dispatch failed:', routeErr.message);
    ...
    if (validated.outputType === 'pending' && validated.jobId) {
      _orchLog(`[Orchestration] Async job queued — job_id=${validated.jobId}`);
    ...
    _orchLog(`[Orchestration] ✓ DONE "${toolName}" — ${elapsed}ms output_type=${outputType} execId=${execId}`);
    console.groupEnd();
...
function _runFrontendFallback(toolName, capability, payload, err) {
  ...
  if (ORCH_CONFIG.NO_CANVAS_FALLBACK_CAPS.has(capability)) {
    ...
    _orchWarn(`[Orchestration] Canvas fallback BLOCKED for ${capability} tool "${toolName}" — showing error instead`);
    return false;
  }

  _orchLog(`[Orchestration] Canvas fallback permitted for ${capability} tool "${toolName}"`);
  ...
  if (fileToUse && typeof window.applyRealCanvasProcessing === 'function') {
    return window.applyRealCanvasProcessing(fileToUse, toolName, payload && payload.params)
      ...
      .catch(e2 => {
        _orchWarn('[Orchestration] Canvas processing also failed:', e2.message);
...
(function _installOrchestrationGate() {
  ...
  _orchLog('[Orchestration] executeToolSafe gate installed');
})();
...
(function _syncToolCapabilityMap() {
  ...
  _orchLog(`[Orchestration] TOOL_CAPABILITY_MAP synced: ${Object.keys(window.TOOL_CAPABILITY_MAP).length} tools`);
})();
...
/* Renamed _orchLog/_orchWarn to avoid SyntaxError collision with index.html's const _log / const _warn */
function _orchLog(...args)  { console.log('[Luminorbit Orch]', ...args); }
function _orchWarn(...args) { console.warn('[Luminorbit Orch]', ...args); }
/* Aliases kept for backward-compat if anything outside calls the old names */
/* NOTE: Do NOT re-declare _log/_warn here — index.html owns those as const */
...
const ProviderBenchmark = (() => {
  ...
  function record(provider, latencyMs, success, capability) {
    ...
    if (s.callCount >= 5 && s.errorRate > CIRCUIT_TRIP_RATE) {
      if (!_tripped[provider]) {
        ...
        _orchLog(`[ProviderBenchmark] Circuit TRIPPED for ${provider} (errorRate=${s.errorRate.toFixed(2)})`);
      }
    }
  ...
  function isCircuitOpen(provider) {
    ...
    if (Date.now() - trip > CIRCUIT_COOLDOWN_MS) {
      ...
      _orchLog(`[ProviderBenchmark] Circuit RESET for ${provider} after cooldown`);
      return false;
    }
    return true;
  }
    ...
    return new Promise(resolve => {
      ...
      img.onload = () => {
        ...
        if (w > MAX_DIMENSION || h > MAX_DIMENSION) {
          ...
          _orchWarn(`[IntelligentValidator] Large image (${w}×${h}px) — backend will resize`);
        }
...
window.executeOrchestrated = async function executeOrchestratedEnterprise(toolName, userParams, inputFile) {
  if (!toolName) {
    _orchWarn('[Enterprise] executeOrchestrated called without toolName');
    return false;
  }
...
// ── FIX: sync executeToolSafe to the enterprise-enhanced executeOrchestrated ─
// window.LuminorbitOrchestration.executeToolSafe was assigned before the enterprise
// override at line 2436, so it held a stale reference. Re-sync it now.
if (window.LuminorbitOrchestration) {
  window.LuminorbitOrchestration.executeToolSafe = window.executeOrchestrated;
}

_orchLog(`[Enterprise] v${LMN_ORCH_VERSION} systems active: SemanticOrchestrator + ExecutionTelemetry + ProviderBenchmark + PriorityQueue`);
