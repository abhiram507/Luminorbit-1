/**
 * ╔══════════════════════════════════════════════════════════════════════════════╗
 * ║   LUMINORBIT v26 — PRODUCTION ORCHESTRATION ENGINE                         ║
...
const ORCH_CONFIG = {
  ...
  // ── Timeouts (ms) ──────────────────────────────────────────────────────────
  TIMEOUT_FAST:      10000,
  TIMEOUT_STANDARD:  18000,
  TIMEOUT_HEAVY:     40000,
  TIMEOUT_VIDEO:    130000,
  TIMEOUT_ASYNC:    190000,
...
const TOOL_EXECUTION_RULES = {
  'image-gen':         { timeout_ms: 18000,  max_retries: 2, is_heavy: false, async_poll: false },
  'image-enhancement': { timeout_ms: 18000,  max_retries: 2, is_heavy: false, async_poll: false },
  'super-resolution':  { timeout_ms: 40000,  max_retries: 2, is_heavy: true,  async_poll: true  },
  'segmentation':      { timeout_ms: 40000,  max_retries: 2, is_heavy: true,  async_poll: true  },
  'inpainting':        { timeout_ms: 40000,  max_retries: 2, is_heavy: true,  async_poll: true  },
  'restoration':       { timeout_ms: 40000,  max_retries: 2, is_heavy: true,  async_poll: true  },
  'face-processing':   { timeout_ms: 40000,  max_retries: 2, is_heavy: true,  async_poll: true  },
  'style-transfer':    { timeout_ms: 18000,  max_retries: 2, is_heavy: false, async_poll: false },
  'controlnet':        { timeout_ms: 22000,  max_retries: 2, is_heavy: false, async_poll: false },
  'video-gen':         { timeout_ms: 130000, max_retries: 1, is_heavy: true,  async_poll: true  },
  'temporal':          { timeout_ms: 95000,  max_retries: 1, is_heavy: true,  async_poll: true  },
  'compression':       { timeout_ms: 14000,  max_retries: 2, is_heavy: false, async_poll: false },
  'audio-extraction':  { timeout_ms: 14000,  max_retries: 2, is_heavy: false, async_poll: false },
  'audio-sync':        { timeout_ms: 14000,  max_retries: 2, is_heavy: false, async_poll: false },
  'captioning':        { timeout_ms: 14000,  max_retries: 2, is_heavy: false, async_poll: false },
  'visualization':     { timeout_ms: 14000,  max_retries: 2, is_heavy: false, async_poll: false },
  'basic-processing':  { timeout_ms:  9000,  max_retries: 2, is_heavy: false, async_poll: false },

  get(capability) {
    return this[capability] || { timeout_ms: 14000, max_retries: 2, is_heavy: false, async_poll: false };
  },
};
...
const FetchEngine = (() => {
  ...
  async function uploadSafeFetch(url, payload, fileObj, timeoutMs = ORCH_CONFIG.TIMEOUT_UPLOAD) {
    ...
    // Encode file to base64 and inject into payload
    let body = payload;
    if (fileObj) {
      try {
        const b64 = await _fileToBase64(fileObj);
        body = { ...payload, file_data: b64, file_mime: fileObj.type };
      } catch (encErr) {
...
const ResultDispatcher = {
  ...
  _renderImage(src, toolName) {
    ...
    if (img) {
      ...
      img.src           = src;
      img.style.filter  = 'none';    // ← CRITICAL: clear any canvas-filter fake
      img.style.opacity = '1';
      ...
      img.onerror = () => {
        ...
        this.showError('Image preview could not be loaded. The output URL may have expired.');
      };
    }
...
window.executeOrchestrated = async function executeOrchestrated(toolName, userParams, inputFile) {
  ...
  _log(`[Orchestration] ▶ START "${toolName}"`);
  ...
  try {
    ...
    _log(`[Orchestration] ✓ DONE "${toolName}" — ${elapsed}ms output_type=${outputType}`);
    ...
    if (e instanceof OrchError) {
      console.error(`[Orchestration] OrchError "${toolName}" [${e.code}] ${elapsed}ms:`, e.message);
      ResultDispatcher.showError(e.message, e.code);
    } else {
      console.error(`[Orchestration] Unhandled error "${toolName}" ${elapsed}ms:`, e);
...
_log(
  `Orchestration Engine v${LMN_ORCH_VERSION} ready.`,
